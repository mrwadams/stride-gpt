...
from stride_gpt.core.schemas import LLMResponse, SubsystemFinding, TokenUsage
...
class TestSubsystemFindingTokenUsage:
    def test_each_finding_gets_its_own_accumulator(self):
        """``token_usage: TokenUsage = TokenUsage()`` is a mutable default
        that every finding then mutates in place. Pydantic deep-copies it per
        instance; if that ever changed, every subsystem in a run would share
        one accumulator and report the whole run's spend as its own."""
        a = SubsystemFinding(subsystem="A", threats=[])
        b = SubsystemFinding(subsystem="B", threats=[])
        assert a.token_usage is not b.token_usage

        a.token_usage.record(LLMResponse(content="", prompt_tokens=5, completion_tokens=1))
        assert a.token_usage.total_tokens == 6
        assert b.token_usage.available is False
